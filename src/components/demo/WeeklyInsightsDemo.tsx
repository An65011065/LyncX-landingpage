...
const WeeklyInsights = ({ isDarkMode = false }: WeeklyInsightsProps) => {
    ...
    const data = {
        scores: [72, 85, 0, 91, 78, 82, 88], // Sun to Sat, Wed has no data
        screenTime: [
            4.5 * 60 * 60 * 1000, // 4.5 hours
            3.2 * 60 * 60 * 1000, // 3.2 hours
            0, // No data
            2.8 * 60 * 60 * 1000, // 2.8 hours
            5.1 * 60 * 60 * 1000, // 5.1 hours
            3.7 * 60 * 60 * 1000, // 3.7 hours
            4.2 * 60 * 60 * 1000, // 4.2 hours
        ],
    ...
    useEffect(() => {
        const updateChartHeight = () => {
            if (chartContainerRef.current) {
                ...
                setChartHeight(Math.max(availableHeight, 80));
            }
        };
        updateChartHeight();
